...
classdef my_graph
    methods(Static)
        ...
        %% ADD: New function to support lifecycle profiles plot in run_slcm
        function plot_lifecycle_profiles(sim)
            % This function plots average consumption and asset (wealth) profiles by age

            T = max(sim.tsim(:)); % Maximum age
            avg_c = nan(T,1);
            avg_a = nan(T,1);

            for t = 1:T
                idx = sim.tsim == t;
                avg_c(t) = mean(sim.csim(idx), 'omitnan');
                avg_a(t) = mean(sim.asim(idx), 'omitnan');
            end

            % Plotting
            plot(1:T, avg_c, '-o', 'DisplayName', 'Consumption');
            hold on
            plot(1:T, avg_a, '-x', 'DisplayName', 'Wealth');
            hold off
            legend('Location','best')
            xlabel('Age')
            ylabel('Value')
            title('Life Cycle Profiles: Average Consumption and Wealth')
        end
